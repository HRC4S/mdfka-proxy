...
void server::addBlock(int tile, int x, int y) {
    ...
    g_server->SendPacketRaw1(true, 4, packPlayerMovings(&data), 56, 0, ENET_PACKET_FLAG_RELIABLE);
}
    ...
    while (enet_host_service(m_proxy_server, &evt, 0) > 0) {
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
            ...
            case NET_MESSAGE_GAME_PACKET: {
                ...
                if (packet->m_type == 24 || packet->m_type == 16) {
                    gt::send_log("Skipping Items Data..");
...
bool server::setup_client() {
    ...
    m_real_server->usingNewPacket = false;
    if (!m_real_server) {
...
void server::disconnect(bool reset) {
    std::ifstream ip2("ip.txt");
    ip2 >> m_server;
    ip2.close();

    std::ifstream port2("port.txt");
    port2 >> m_port;
    port2.close();
...
void server::SendPacketRaw1(bool client, int a1, void* packetData, size_t packetDataSize, void* a4, int packetFlag) {
    ENetPacket* p;
    auto peer = client ? m_gt_peer : m_server_peer;
    if (peer) {
...
void punch(int x, int y) {
    ...
    memcpy(packPlayerMovings(&data) + 16, &waterspeed, 4);
    g_server->SendPacketRaw1(false, 4, packPlayerMovings(&data), 56, 0, ENET_PACKET_FLAG_RELIABLE);
}
